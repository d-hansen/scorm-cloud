...
describe "Rustici Web Service API" do
	...
	describe ScormCloud::ScormCloud.new($scorm_cloud_appid,$scorm_cloud_secret).course do
		...
		it { should respond_to(:get_assets).with(2).arguments }
		it { should respond_to(:get_file_structure).with(1).argument }
		it { should respond_to(:get_metadata).with(3).arguments }
